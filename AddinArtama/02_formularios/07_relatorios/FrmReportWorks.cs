...
namespace AddinArtama {
  public partial class FrmReportWorks : LmSingleForm {
    ...
    private void BtnLoad_Click(object sender, EventArgs e) {
      ...
      try {
        ...
        if (swModel.GetType() == (int)swDocumentTypes_e.swDocASSEMBLY) {
          ...
          _reports = ReportWorks.GetReport();
    ...
    private void BtnSalvar_Click(object sender, EventArgs e) {
      ...
      if (Sw.App.ActiveDoc != null && !string.IsNullOrEmpty(((ReportWorks)_reports[0]).PathName)) {
      ...
      try {
        if (Open3D()) {
          ...
          if (!string.IsNullOrEmpty(((ReportWorks)dgv.Grid.CurrentRow.DataBoundItem).Codigo)) {
            ...
            if (!ReportWorks.UpdateCutList(nCutList, (ReportWorks)dgv.Grid.CurrentRow.DataBoundItem))
