      ...
      try {
        ...
        for (int i = lStartRow; i < swTableAnnotation.TotalRowCount; i++) {
          ...
          if (vModelPathNames != null) {
            ...
            if (produtoErp.TipoComponente == TipoComponente.Peca) {
              ...
              if (changeCutList && produtoErp.ItensCorte.Count > 1) {
                ...
                if (!string.IsNullOrEmpty(produtoErp.ItensCorte[0].Operacao)) {
                  ...
                  ListaCorte.DeletarPropriedadeDaLista(swModel, produtoErp.ItensCorte[0], "Operação");

                  var swModelDocExt = swModel.Extension;
                  var swCustPropMngr = swModelDocExt.get_CustomPropertyManager("");
                  swCustPropMngr.Add3("Operação", (int)swCustomInfoType_e.swCustomInfoText, produtoErp.Operacao, (int)swCustomPropertyAddOption_e.swCustomPropertyDeleteAndAdd);
                  swModel.Save();
                }
              }
            }
            ...
            if (_listaProduto.Any(x => x.Name == produtoErp.Name && x.Configuracao == produtoErp.Configuracao) ||
              (produtoErp.CodComponente.StartsWith("10") || produtoErp.CodComponente.StartsWith("20")))
              continue;
